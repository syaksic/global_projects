...
def iterate_BF(users,graph,rates):
	...
	for k in range(len(graph['degree']['index'])):
		...
		W_IS_k[k]=aux*k*rates['accepting_rate']*users['i_k_t'][k]*Theta_k
		W_IR_k[k]=aux*k*rates['reffusing_rate']*users['i_k_t'][k]*Theta_k
		W_SR_k[k]=aux*(k*rates['stifling_rate']*users['s_k_t'][k]*Theta_k+rates['forgetting_rate']*users['s_k_t'][k])
		W_RS_k[k]=aux*(k*rates['persuading_rate']*users['r_k_t'][k]*Theta_k+rates['remembering_rate']*users['r_k_t'][k])
