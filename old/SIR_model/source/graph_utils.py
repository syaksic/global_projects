...
def degree_dist(degree_sequence,graph_size):
	...
	return{'index':index,'count':count,'prob':prob, 'popt':popt,'kmean':kmean,'bf':degree_dist_fit(index,popt[0],popt[1]),'teo':degree_dist_fit(index,np.float64(3.0),np.float64(1.0))}
...
def degree_corr(indices,counts,Graph,kmean):
	...
	teo=degree_degree_cor_func_fit(indices,np.float64(3.0),np.float64(1.0),np.float64(kmean))
